...
async (conn, mek, m, { from, reply }) => {
    try {
        ...
        const flirtMessage = `${res.data.result}`;
...
async (conn, mek, m, { from, reply }) => {
    try {
        const shizokeys = 'shizo';
        const res = await axios.get(`https://shizoapi.onrender.com/api/texts/truth?apikey=${shizokeys}`);
        ...
        const truthText = `${res.data.result}`;
        await conn.sendMessage(from, { 
            text: truthText, 
            mentions: [m.sender] 
        }, { quoted: mek });
...
async (conn, mek, m, { from, reply }) => {
    try {
        ...
        // Fetch dare text from the API using axios
        const res = await axios.get(`https://shizoapi.onrender.com/api/texts/dare?apikey=${shizokeys}`);
        ...
        const dareText = `${res.data.result}`;
...
async (conn, mek, m, { from, reply }) => {
    try {
        // Fetch pickup line from the API using axios
        ...
        await conn.sendMessage(from, { text: pickupLine }, { quoted: mek });
...
async (conn, mek, m, { from, isGroup, text, reply }) => {
    try {
        ...
        const userChar = [
            ...
            "MF💀",
            "Arrogant🤦‍♂️",
            "We'll rather not say 🏳️ my owner will query me💀 for using such foul languages",
        ];
...
cmd({
  ...
  try {
    const botOwner = conn.user.id.split(":")[0] + '@s.whatsapp.net'; // Get bot owner's full JID
    ...
    for (let i = 0; i < count; i++) {
      await conn.sendMessage(m.from, { text: message }, { quoted: mek });
...
cmd({
  ...
  try {
    ...
    const message = `${inputText}${readMore}`; // Only shows "read more" break

    await conn.sendMessage(
      m.chat, // ✅ correct property
      { text: message },
      { quoted: m }
    );
  } catch (error) {
